...
class Network():
    ...
    def get_layer_output(self, x, labels, layer_name):
        self.model.eval()
        with torch.no_grad():
            layer_idx = list(self.model._modules).index(layer_name)
            intermediate_model = nn.Sequential(*list(self.model.children())[:layer_idx+1]).to(device)
...
def get_important_filters(model, train_loader):
    ...
    sorted_filters = sort(most_important_filters, conv_layers, 0.75)
...
def train_model(model, train_loader, test_loader, epochs=250, lr=0.1, lr_drop=20, base=False, selected_filters=None, conv_layers = None):
    global pruning
    best_accuracy=0
    best_model=None
    pruning += 1
...
def prune(model):
    ...
    # base_model=model
    # base_model=train(base_model, criterion, optimizer, scheduler, train_loader, test_loader, params["num_epochs"], params["lambda_l1"],start=True )
    ...
    # model.save_weights("vgg16_pruned_model.h5")
